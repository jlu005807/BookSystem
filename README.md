...
### 功能特性
- **自动日志记录**：系统运行状态、数据库连接、用户操作等自动记录
- **分级日志**：支持 DEBUG、INFO、WARN、ERROR 四个级别
- **文件管理**：按日期自动分割日志文件，自动清理7天前的旧日志
- **详细记录**：包含时间戳、操作类型、错误堆栈等详细信息

### 日志文件位置
- 日志文件保存在项目根目录的 `logs/` 文件夹下
- 文件名格式：`bookSystem_YYYY-MM-DD.log`
- 例如：`bookSystem_2024-01-15.log`
...
### 查看日志
- 直接用文本编辑器（如记事本、Notepad++、VSCode等）打开 `logs/` 文件夹下对应日期的日志文件即可。
...
1. 进入 `out/artifacts/bookSystem_jar/` 目录（或你实际的jar包输出目录）。
2. 确保已安装 Java 8 及以上版本。
3. 确保 `logs/` 文件夹和 `resource/` 资源文件夹与 jar 包在同一目录下，或在 jar 包运行目录下存在。
4. 启动命令示例：
   ```bash
   java -jar bookSystem.jar
   ```
   > 其中 `bookSystem.jar` 为实际生成的 jar 包文件名。

### 注意事项
- 运行前请确保 MySQL 数据库已启动，且数据库配置正确。
- 第一次运行会自动在当前目录下生成 `logs/` 日志文件夹。
- 日志文件和资源文件需与 jar 包在同一目录，或通过参数指定。
- 如遇数据库连接失败、资源找不到等问题，请先查看 `logs/` 文件夹下的日志文件，按日志内容排查。
- 若需更换数据库配置，请修改源码中的 `src/Main.java` 或提供外部配置支持。 